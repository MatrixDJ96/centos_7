...
if [ "${1}" = "--docker-install" ]; then
  ...
  # Build base image
  docker image rm -f local/rhel9 >/dev/null 2>&1
  ...
  docker run -t -d -v $(pwd):/vagrant --name rhel9 local/rhel9 bash
  ...
  # Pull RHEL9 image
  docker pull matrixdj96/rhel9:latest
  docker tag matrixdj96/rhel9:latest local/rhel9
elif [ "${1}" = "--docker-push" ]; then
  # Push RHEL9 image
  docker login -u matrixdj96 docker.io
  docker tag local/rhel9 matrixdj96/rhel9:latest
  docker push matrixdj96/rhel9:latest
  ...
  echo -e "generateHosts=true" >> /etc/wsl.conf
