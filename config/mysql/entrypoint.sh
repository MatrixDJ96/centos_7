...
# https://github.com/docker-library/mysql/blob/master/8.4/docker-entrypoint.sh
...
if [ "$(id -u)" = "0" ]; then
	# Fix log file permissions for mysql user access
	install /dev/null -m0640 -omysql -gmysql "/var/log/mysqld.log"
	install -d -m0751 -omysql -gmysql /var/lib/mysql

	mysql_note "Switching to dedicated user 'mysql'"
	exec runuser -u mysql -- "$BASH_SOURCE" "$@"
fi
